...
// Function to display RFID data on the OLED screen
void displayData() {
  // Clear the screen
  screen.clear();

  // Display the current state (emulating or reading)
  if (emulating) {
    screen.drawString(0, 0, "Emulating card:");
  }
  else {
    screen.drawString(0, 0, "Reading card:");
  }

  // Display the current card index
  screen.drawString(0, 10, "Card " + String(currentCard + 1) + " of 4");

  // Display the RFID data
  screen.drawString(0, 20, "Data:");
  for (int i = 0; i < 4; i++) {
    screen.drawString(0, 30 + i * 10, "Block " + String(i) + ": " + byteToHex(rfidData[currentCard][i]));
  }
